...
class DiscordBot(commands.Bot):
    def __init__(self):
        ...
        super().__init__(
            command_prefix="/", # Changed command prefix to /
            description=BOT_DESCRIPTION,
            intents=intents
        )
        self.tree.sync()  # Sync slash commands with Discord
    ...
    async def setup_hook(self):
        ...
        await self.tree.sync()  # Ensure commands are synced on startup
    ...
    async def on_ready(self):
        ...
        await self.change_presence(
            activity=discord.Activity(
                ...
                name=f"/commands | {BOT_NAME}" # Changed prefix in status message
            ),
    ...
    async def rotate_status(self):
        ...
        statuses = [
            (discord.ActivityType.listening, f"/commands | {BOT_NAME}"), # Changed prefix
            (discord.ActivityType.playing, f"in {len(self.guilds)} servers"),
            (discord.ActivityType.watching, "for your messages"),
            (discord.ActivityType.listening, f"/help | AI-powered"), # Changed prefix
    ...
    async def on_guild_join(self, guild):
        ...
        if welcome_channel:
            ...
            embed.add_field(
                ...
                value=f"• Use `/register` to register with the bot\n" # Changed prefix
                      f"• Try `/chat` to talk with me\n" # Changed prefix
                      f"• Check `/commands` for the full list of commands", # Changed prefix
                inline=False
            )
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.CommandNotFound):
            await ctx.send(f"Command not found. Use `/commands` to see available commands.") # Changed prefix
    ...
    async def on_message(self, message):
        ...
        if self.user.mentioned_in(message) and not message.mention_everyone:
            ...
            await message.channel.send(
                f"Hello {message.author.mention}! Use `/commands` to see what I can do." # Changed prefix
            )